...
export class MassaAddress {
  ...
  static toU256(addr: string): U256 {
    ...
    /*
    this is implementation from node_modules/as-bignum/assembly/integer/u256.ts
    lo1 is at index 0.
     ...
     */
    return {
      lo1: BigInt('0x' + Buffer.from(pkb32.slice(0, 8)).toString('hex')),
      lo2: BigInt('0x' + Buffer.from(pkb32.slice(8, 16)).toString('hex')),
      hi1: BigInt('0x' + Buffer.from(pkb32.slice(16, 24)).toString('hex')),
      hi2: BigInt('0x' + Buffer.from(pkb32.slice(24, 32)).toString('hex')),
    };
  }
  ...
  static sort_old(addr1: string, addr2: string): number {
