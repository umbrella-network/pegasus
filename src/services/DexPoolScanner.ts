...
export class DexPoolScanner {
  ...
  constructor(chainId: ChainsIds, dexProtocolName: DexProtocolName) {
    ...
    this.dexProtocol = DexProtocolFactory.create(chainId, dexProtocolName, settings, this.provider);
  ...
  async run(): Promise<Result> {
    ...
    if (!this.provider) {
      throw new Error(`${this.logPrefix} getToBlock: no provider`);
    }
  ...
  private getToBlock(fromBlock: number, latestBlock: number): number {
    ...
    // Math.trunc(blocksToScan / this.step) * this.step;
