...
@injectable()
export class MassaDeviationDispatcher extends DeviationDispatcher {
  ...
  protected async resolveGasMetrics(): Promise<GasEstimation | undefined> {
    this.logger.warn('[MassaDeviationDispatcher] resolveGasMetrics: TODO');
    return undefined;
  }

  protected async calculatePayableOverrides(props?: {nonce?: number, data?: unknown}): Promise<PayableOverrides> {
    this.logger.warn('[MassaDeviationDispatcher] calculatePayableOverrides: TODO');
  ...
  protected async cancelPendingTransaction(prevGasPrice: number | undefined, timeout: number): Promise<boolean> {
    this.logger.warn('[MassaDeviationDispatcher] cancelPendingTransaction: not supported');
    return false;
  }
}
