...
/**
 * This class came from uniswapV3 SDK activity-liquidity.ts example
 * https://github.com/Uniswap/examples/blob/main/v3-sdk/pool-data/src/libs/active-liquidity.ts
 * https://docs.uniswap.org/sdk/v3/guides/advanced/active-liquidity
 */

@injectable()
export class ActiveLiquititySDK {
  async apply(
    tickCurrent: number,
    poolLiquidity: JSBI,
    tickSpacing: number,
    ...
    const processedTicks = this.processTicks(
      tickCurrent,
      poolLiquidity,
      tickSpacing,
      token0,
    ...
    const barTicks = await Promise.all(
      processedTicks.map(async (tick: TickProcessed) => {
        return this.calculateLockedLiqudity(tick, token0, token1, tickSpacing, feeTier);
      }),
    );
  ...
  private processTicks(
    ...
    const subsequentTicks: TickProcessed[] = this.computeInitializedTicks(
      activeTickProcessed,
      numSurroundingTicks,
      tickSpacing,
      Direction.ASC,
    ...
    const previousTicks: TickProcessed[] = this.computeInitializedTicks(
      activeTickProcessed,
      numSurroundingTicks,
      tickSpacing,
      Direction.DESC,
  ...
  private computeInitializedTicks(
  ...
  private async calculateLockedLiqudity(
    tick: TickProcessed,
    token0: Token,
    token1: Token,
    tickSpacing: number,
