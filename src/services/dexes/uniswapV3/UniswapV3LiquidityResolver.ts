...
export class UniswapV3LiquidityResolver {
  ...
  @inject(LiquidityService) liquidityService!: LiquidityService;
  ...
  async apply(chainId: ChainsIds) {
    ...
    const uniswapV3Params = this.feedDataService.getParamsByFetcherName<UniswapV3Param>(feeds, 'UniswapV3');
  ...
  private async getFeeds(chainId: ChainsIds): Promise<LeavesAndFeeds> {
    const dataTimestamp = this.timeService.apply();
    const data = await this.feedDataService.apply(dataTimestamp, FeedsType.CONSENSUS);

    if (data.rejected) {
      this.logger.info(`${this.logPrefix}[${chainId}] rejected: ${data.rejected}`);
    }

    return data.feeds as LeavesAndFeeds;
  }

  private async getTokens(token0: string, token1: string, chainId: ChainsIds) {
    ...
    if (!tokenA) {
      throw new Error(`${this.logPrefix}[${chainId}] token0 not found`);
    }

    if (!tokenB) {
      throw new Error(`${this.logPrefix}[${chainId}] token1 not found`);
    }
  ...
  private async processParams(uniswapV3Params: UniswapV3Param[], chainId: ChainsIds) {
    await Promise.all(
      uniswapV3Params.map(async (param) => {
        ...
        if (pools.length === 0) {
          this.logger.error(`${this.logPrefix}[${chainId}] pool not found`);
          return;
        }
  ...
  private async getLiquidities(pool: UniswapV3Pool, token0: Token, token1: Token, chainId: ChainsIds) {
    const liquidity = await this.liquidityService.apply(pool.address, token0, token1, pool.fee, chainId);
    return liquidity;
  }
