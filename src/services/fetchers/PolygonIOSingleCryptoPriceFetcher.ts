...
export class PolygonIOSingleCryptoPriceFetcher extends BasePolygonIOSingleFetcher implements FeedFetcherInterface {
  ...
  async apply(params: PolygonIOSingleCryptoInputParams[], options: FeedFetcherOptions): Promise<FetcherResult> {
  ...
  private async fetchPrice({fsym, tsym}: PolygonIOSingleCryptoInputParams): Promise<SinglePriceResponse | undefined> {
    try {
  ...
  private parseResponse(response: SinglePriceResponse[]): ParsedResponse[] {
    return response
      .map(({symbol, last}) => {
        ...
        return {symbol, price: value, timestamp: last.timestamp};
      })
